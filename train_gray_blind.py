...
#  python train_gray_blind.py -c 0
...
def main():
    ...
    weight_path = "weights"
    if opt.checkpoint != 0:
        model.load_state_dict(torch.load(os.path.join(weight_path, "model_gray_B_%d.pth" %(opt.checkpoint))))
    ...
    for epoch in range(opt.checkpoint, opt.epochs):
        ...
        for step, x in enumerate(loader_train):
            ...
            if step % 400 == 0 and step != 0:
                ...
                with open('log_gray_B.txt', 'a+') as f:
        ...
        if epoch % 1 == 0:
            ...
            torch.save(model.state_dict(), os.path.join(weight_path, "model_gray_B_%d.pth" %(epoch+1)))
            scheduler.step()
