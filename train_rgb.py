...
def main():
    ...
    if opt.checkpoint != 0:
        ...
        viz.line([34], [opt.checkpoint], win='rgb_train_psnr', opts=dict(title='rgb_train_psnr'))
        model.load_state_dict(torch.load(os.path.join(weight_path, "model_rgb_L%d_%d.pth" %(opt.noiseL,opt.checkpoint))))
    ...
    for epoch in range(opt.checkpoint, opt.epochs):
        ...
        for step, x in enumerate(loader_train):
            ...
            if step % 400 == 0 and step != 0:
                ...
                with open('log_rgb_L%d.txt' % (opt.noiseL), 'a+') as f:
        ...
        if epoch % 1 == 0:
            ...
            torch.save(model.state_dict(), os.path.join(weight_path, "model_rgb_L%d_%d.pth" %(opt.noiseL,epoch+1)))
            scheduler.step()
